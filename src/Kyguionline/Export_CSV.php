...
class Export_CSV {
  ...
  public function array2csv(array &$array, $df) {
    ...
    $heading = array();
    //$unwanted = array('_gap_file', 'cfdb7_', 'your-');
    $unwanted = array('_gap_file');
  ...
  public function download_csv_file($form_id) {
    ...
    if (isset($_REQUEST['csv']) && isset($_REQUEST['nonce'])) {
      ...
      $this->download_send_headers("gap-kyguionline-" . date("Y-m-d") . ".csv");
      ...
      for ($p = 0; $p <= $total_query; $p++) {
        ...
        foreach ($results as $result):
          ...
          $cfdb7_dir_url = $upload_dir['baseurl'] . '/gap_kyguionline_uploads';

          foreach ($resultTmp as $key => $value):
            $matches = array();

            if (!in_array($key, $heading_key))
              continue;
            if ($rm_underscore)
