...
class Details {
  ...
  public function __construct() {
    ...
    if (in_array($this->form_type, array('online', 'offline')))
      echo '<form method="post">';
    if ($this->form_type == 'online') //Ky gui online
      $this->form_details_online($results);
    elseif ($this->form_type == 'offline') //Dat lich cua hang
      $this->form_details_offline($results);
    else
      $this->form_details_page_others($results);

    if (in_array($this->form_type, array('online', 'offline')))
      echo '</form>';

    if (isset($_POST['btnApprove'])) {
      file_put_contents('D:/btnApprove.txt', 'CHAP NHAN');
    ...
    if (isset($_POST['btnReject'])) {
      file_put_contents('D:/btnReject.txt', 'btnReject');
    }
  }

  public function form_details_online($results = '') {
      ...
      <div id="welcome-panel" class="cfdb7-panel">
        <div class="cfdb7-panel-content">
          <div class="welcome-panel-column-container">
            ...
            $this->form_controls($results[0]->status);
            ?>
          </div>
        </div>
      </div>
  ...
  public function form_details_offline($results) {
    $cfdb7_dir_url = $this->cfdb7_dir_url;
    ?>
  ...
  public function form_controls($status) {
    if ($status == "new") {
      ?>
  ...
  public function form_details_page_others($results = '') {
    if (empty($results)) {
      wp_die($message = 'Not valid contact form');
    }
