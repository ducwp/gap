...
class Kyguionline {
  ...
  private function __construct() {
    $this->form_id = 2686;
    $this->form_hash = 'e25bfe3';
    
    add_action('wpcf7_before_send_mail', [$this, 'cf7_before_send_mail']);
  }

  function cf7_before_send_mail($form_tag) {

    //file_put_contents("D:/form.txt", json_encode(get_class_methods($form_tag)));
    file_put_contents("D:/form-hash.txt", json_encode($form_tag->hash()));

    if ($form_tag->hash() != $this->form_hash)
      return;


    global $wpdb;
    $cfdb = apply_filters('cf7_gap_database', $wpdb);
    $table_name = $cfdb->prefix . 'gap_kyguionline';
    $upload_dir = wp_upload_dir();
    $gap_dirname = $upload_dir['basedir'] . '/gap_kyguionline_uploads';
    $time_now = time();
    ...
    if ($submission) {
      ...
      foreach ($data as $key => $d) {
        ...
        if (in_array($key, $uploaded_files)) {
          ...
          $key = sanitize_text_field($key);
          $form_data[$key . 'gap_file'] = $file_name;
        }
      }
