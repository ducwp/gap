...
if ($user_data['level'] != 'member') {
  ...
  if($user_data['point'] > 0) printf(__('Chúc mừng! Bạn đã kiếm được <b>%s</b> điểm.'), $user_data['point']);
  else echo 'Bạn chưa có điểm nào, hãy mua hàng để tích lũy điểm.';
echo '</p>';
...
if ($user_data['level'] !== 'member') {
  ...
  if (!empty($terms)) {
    ...
    if (!empty($cat_ids)) {
      ...
      echo '<ul class="vip_cats"><li><b>Đặc quyền danh mục sản phẩm dành riêng cho VIP/ VVIP</b>:</li>';
